...
public abstract class ImageWorker {
	...
	private ArrayList<WeakReference<BitmapWorkerTask>> mTaskList = new ArrayList<WeakReference<BitmapWorkerTask>>();
	 ...
	 * {@link DandAsyncTask} will be created to asynchronously load the bitmap.
	 * 
	 * @param data
	 ...
	 */
	public boolean loadImage(Object data, ImageView imageView,
			boolean needDownload) {
		return loadImage(data, imageView, mLoadingBitmap, needDownload);
	}

	public boolean loadImage(Object data, ImageView imageView,
			Bitmap loadingBitmap, boolean needDownload) {
		Bitmap bitmap = null;
		...
		if (bitmap != null) {
			...
			if (needDownload && cancelPotentialWork(data, imageView)) {
	...
	private void executeTask(BitmapWorkerTask task, Object data) {
		if (mTaskList.size() >= MAX_TASK_NUM) {
			BitmapWorkerTask bitmapWorkerTask = mTaskList.get(0).get();
		...
		mTaskList.add(new WeakReference<ImageWorker.BitmapWorkerTask>(task));
	...
	public boolean loadImage(Object data, ImageView imageView, int loadingRes,
			boolean needDownload) {
		Bitmap loadingBitmap = BitmapFactory.decodeResource(
				mContext.getResources(), loadingRes);

		return loadImage(data, imageView, loadingBitmap, needDownload);
	}
	 ...
	 *            The ImageView to bind the downloaded image to.
	 */
	public boolean loadImage(int num, ImageView imageView) {
		if (mImageWorkerAdapter != null) {
			return loadImage(mImageWorkerAdapter.getItem(num), imageView, true);
		} else {
	...
	/**
	 * The actual VdiskAsyncTask that will asynchronously process the image.
	 */
	private class BitmapWorkerTask extends DandAsyncTask<Object, Void, Bitmap> {
		private Object data;
